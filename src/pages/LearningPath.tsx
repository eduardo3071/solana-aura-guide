...
import {
  ...
  Circle, BookOpen, Brain, ArrowRight, Sparkles, RotateCcw,
  Home,
} from "lucide-react";
...
const LearningPath = () => {
  ...
  useEffect(() => {
    ...
    const stepContext = currentStep === 0
  ...
  const handleTermNavigate = useCallback((term: GlossaryTerm) => {
    ...
    <div className="min-h-[calc(100vh-3.5rem)] max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-foreground mb-1">
          {t("learn.title")}: <span className="gradient-text">{startTerm.term}</span>
        </h1>
        <p className="text-sm text-muted-foreground">
          {pathData.steps.length} {t("learn.steps_label")} · {t("learn.based_on_graph")}
        </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className={`font-medium truncate ${active ? "text-foreground" : ""}`}>
                      {s.term.term}
                    </p>
                  </div>
                </button>
            ...
            {step && (
              ...
              >
                ...
                <div className="bg-card border border-border rounded-xl p-6 mb-4">
                  ...
                  <p className="text-sm text-foreground/80 leading-relaxed">{step.term.definition}</p>
                </div>
                ...
                <div className="bg-card border border-border rounded-xl p-6 mb-4">
                  <div className="flex items-center gap-1.5 mb-3">
                    ...
                    <span className="text-xs font-semibold text-primary">{t("learn.explanation")}</span>
                  </div>
                  ...
                  {loading && !explanation ? (
                    ...
                    <div className="text-sm text-foreground/80 leading-relaxed [&_pre]:bg-[hsl(150_60%_10%)] [&_pre]:border [&_pre]:border-emerald-500/20 [&_pre]:rounded-lg [&_pre]:p-3 [&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre_code]:text-emerald-400 [&_pre_code]:text-[11px] [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_code]:text-primary [&_code]:bg-primary/10 [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded [&_code]:text-[11px] [&_strong]:text-foreground [&_p]:mb-2 [&_p:last-child]:mb-0">
                      <TermHighlightedMarkdown content={explanation} onTermClick={handleTermNavigate} />
                    </div>
                  )}
                </div>
