...
import {
  Brain, Zap, Globe, ArrowRight, CheckCircle2, XCircle,
  RotateCcw, Trophy, Lightbulb, BookOpen,
} from "lucide-react";
...
export function SmartQuiz({ term, onNavigate, onOpenGraph, onExplainCode }: SmartQuizProps) {
  ...
  const [phase, setPhase] = useState<"config" | "loading" | "quiz" | "results">("config");
  ...
  const difficultyOptions: { value: Difficulty; label: string; icon: React.ReactNode }[] = [
    { value: "beginner", label: t("quiz.beginner" as any), icon: <BookOpen className="h-3 w-3" /> },
    { value: "intermediate", label: t("quiz.intermediate" as any), icon: <Brain className="h-3 w-3" /> },
    { value: "advanced", label: t("quiz.advanced" as any), icon: <Zap className="h-3 w-3" /> },
  ];

  const modeOptions: { value: Mode; label: string; icon: React.ReactNode }[] = [
    { value: "concept", label: t("quiz.mode_concept" as any), icon: <BookOpen className="h-3 w-3" /> },
    { value: "connections", label: t("quiz.mode_connections" as any), icon: <Globe className="h-3 w-3" /> },
    { value: "real-world", label: t("quiz.mode_realworld" as any), icon: <Zap className="h-3 w-3" /> },
  ];
    ...
    return (
      <div className="bg-gradient-to-br from-violet-500/5 to-primary/5 border border-violet-500/20 rounded-lg p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-foreground">{t("quiz.title" as any)}</span>
        </div>
        <p className="text-[11px] text-muted-foreground">{t("quiz.description" as any)}</p>
    ...
    return (
      <div className="bg-gradient-to-br from-violet-500/5 to-primary/5 border border-violet-500/20 rounded-lg p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-foreground">{t("quiz.generating" as any)}</span>
        </div>
    ...
    return (
      ...
      >
        <div className="text-center space-y-2">
          <Trophy className={`h-8 w-8 mx-auto ${percentage >= 70 ? "text-yellow-400" : "text-muted-foreground"}`} />
          <h3 className="text-sm font-bold text-foreground">{t("quiz.complete" as any)}</h3>
        ...
        <div className="bg-secondary/50 rounded-lg p-3 space-y-1.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-semibold text-foreground">{t("quiz.insights" as any)}</span>
          </div>
            ...
            <div className="space-y-1">
              {reviewTerms.slice(0, 4).map(rt => (
                ...
                >
                  ...
                  <span className="text-[11px] font-medium text-foreground truncate">{rt.term}</span>
                </button>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="bg-gradient-to-br from-violet-500/5 to-primary/5 border border-violet-500/20 rounded-lg p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <Progress value={((currentQ + (answered ? 1 : 0)) / questions.length) * 100} className="h-1.5 flex-1 mx-3" />
      ...
      >
        <p className="text-xs font-semibold text-foreground leading-relaxed mb-3">{q.question}</p>

        {/* Options */}
        <div className="space-y-1.5">
          {q.options.map((opt, idx) => {
            const letter = String.fromCharCode(65 + idx);
            let optionClass = "bg-secondary/50 border-transparent hover:bg-surface-hover hover:border-primary/10";
            if (answered) {
              if (idx === q.correct) {
            ...
            return (
              ...
              >
                ...
                {answered && idx === q.correct && <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400 shrink-0" />}
                {answered && idx === selectedOption && idx !== q.correct && <XCircle className="h-3.5 w-3.5 text-red-400 shrink-0" />}
              </button>
            );
          })}
        </div>
      ...
      <AnimatePresence>
        {answered && (
          <motion.div
            ...
            className="space-y-2"
          >
            <div className={`rounded-lg p-3 ${selectedOption === q.correct ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-red-500/10 border border-red-500/20"}`}>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[11px] font-semibold text-foreground">
                  {selectedOption === q.correct ? t("quiz.correct" as any) : t("quiz.incorrect" as any)}
                </span>
              </div>
              <p className="text-[11px] text-foreground/80 leading-relaxed">{q.explanation}</p>
            </div>
