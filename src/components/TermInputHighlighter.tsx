...
import { allTerms, GlossaryTerm } from "@/lib/solana-glossary";
...
export function detectTermsInText(text: string): { term: GlossaryTerm; start: number; end: number }[] {
  if (!text.trim()) return [];
  ...
  for (const entry of termLookup) {
    ...
    while ((idx = lower.indexOf(entry.lower, idx)) !== -1) {
      ...
      const after = idx + entry.lower.length >= text.length || /\W/.test(text[idx + entry.lower.length]);
      if (before && after) {
        // Check no overlap
        let overlap = false;
        ...
        if (!overlap) {
          matches.push({ term: entry.term, start: idx, end: idx + entry.lower.length });
 ...
 */
export function TermInputHighlighter({ text, onTermClick, className }: TermInputHighlighterProps) {
